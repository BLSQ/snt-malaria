...
describe('normalizeSelection', () => {
    it('defaults to "all" mode with no ids for a missing/empty value', () => {
        ...
        expect(normalizeSelection({})).toEqual({ mode: 'all', ids: [] });
    });

    it('keeps an explicit "none" mode', () => {
        expect(normalizeSelection({ mode: 'none' })).toEqual({
            mode: 'none',
            ids: [],
        });
    });
    ...
    it('falls back to "all" for any mode value other than "none"', () => {
        expect(normalizeSelection({ mode: 'bogus' })).toEqual({
            mode: 'all',
            ids: [],
        });
    });
    ...
    it('coerces numeric-looking values, dedupes, and drops values that are not', () => {
        expect(
            normalizeSelection({
                mode: 'none',
                ids: [1, '2', 2, 'x', undefined],
            }),
        ).toEqual({ mode: 'none', ids: [1, 2] });
    });
...
describe('isOrgUnitSelected', () => {
    it('under "all", selects everything except the ids list', () => {
        ...
        expect(isOrgUnitSelected(selection, 1)).toBe(true);
        expect(isOrgUnitSelected(selection, 2)).toBe(false);
    });

    it('under "none", selects only the ids list', () => {
        const selection = { mode: 'none' as const, ids: [2] };
        expect(isOrgUnitSelected(selection, 1)).toBe(false);
        expect(isOrgUnitSelected(selection, 2)).toBe(true);
    });
});
...
describe('isDistrictPicked', () => {
    it('reflects raw ids membership regardless of mode', () => {
        expect(isDistrictPicked({ mode: 'all', ids: [5] }, 5)).toBe(true);
        expect(isDistrictPicked({ mode: 'none', ids: [5] }, 5)).toBe(true);
        expect(isDistrictPicked({ mode: 'all', ids: [5] }, 6)).toBe(false);
    });
});

describe('toggleOrgUnit', () => {
    it('adds an id not yet in the list', () => {
        expect(toggleOrgUnit({ mode: 'all', ids: [1] }, 2)).toEqual({
            mode: 'all',
            ids: [1, 2],
        });
    });
    ...
    it('removes an id already in the list', () => {
        expect(toggleOrgUnit({ mode: 'all', ids: [1, 2] }, 1)).toEqual({
            mode: 'all',
            ids: [2],
        });
    });
...
describe('setSelectionMode', () => {
    it('returns the same reference when the mode is unchanged', () => {
        ...
        expect(setSelectionMode(selection, 'all')).toBe(selection);
    });
    ...
    it('flips mode while keeping the same ids, changing what ends up selected', () => {
        ...
        expect(next).toEqual({ mode: 'none', ids: [1, 2] });
        // Under "all" those ids were EXCLUDED; under "none" the same ids are now the only INCLUDED ones.
        expect(resolveSelectedOrgUnitIds(selection, [1, 2, 3])).toEqual([3]);
        expect(resolveSelectedOrgUnitIds(next, [1, 2, 3])).toEqual([1, 2]);
    });
});

describe('resetSelectionOverrides', () => {
    it('clears ids while keeping the given mode', () => {
        expect(resetSelectionOverrides('none')).toEqual({
            mode: 'none',
            ids: [],
        });
    });
...
describe('DEFAULT_ORG_UNIT_SELECTION', () => {
    it('is a no-op selection (mode "all", nothing excluded)', () => {
        expect(DEFAULT_ORG_UNIT_SELECTION).toEqual({
            mode: 'all',
            ids: [],
        });
    });
