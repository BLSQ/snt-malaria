...
class ScenarioRuleAPITestCase(ScenarioRulesTestBase):
    ...
    def test_has_custom_name_round_trip(self):
        """
        has_custom_name should default to False on create, be readable on list/retrieve,
        and toggleable on patch.
        """
        self.client.force_authenticate(user=self.user_with_full_perm)
        ...
        create_response = self.client.post(
            self.BASE_URL,
            {
                ...
                "matching_criteria": {"all": True},
                "intervention_properties": [
                    {"intervention": self.intervention_chemo_iptp.id, "coverage": 0.5},
                ],
            },
        )
        result = self.assertJSONResponse(create_response, status.HTTP_201_CREATED)
        new_rule_id = result["id"]
        self.assertFalse(ScenarioRule.objects.get(id=new_rule_id).has_custom_name)

        list_response = self.client.get(self.BASE_URL, {"scenario": self.scenario.id})
        list_result = self.assertJSONResponse(list_response, status.HTTP_200_OK)
        new_rule_in_list = next(item for item in list_result if item["id"] == new_rule_id)
        self.assertIn("has_custom_name", new_rule_in_list)
        self.assertFalse(new_rule_in_list["has_custom_name"])

        retrieve_response = self.client.get(f"{self.BASE_URL}{new_rule_id}/")
        retrieve_result = self.assertJSONResponse(retrieve_response, status.HTTP_200_OK)
        self.assertFalse(retrieve_result["has_custom_name"])

        patch_response = self.client.patch(
            f"{self.BASE_URL}{new_rule_id}/",
            {"name": "My Custom Name", "has_custom_name": True},
        )
        patched = self.assertJSONResponse(patch_response, status.HTTP_200_OK)
        self.assertTrue(patched["has_custom_name"])

        reset_response = self.client.patch(
            f"{self.BASE_URL}{new_rule_id}/",
            {"has_custom_name": False},
        )
        reset_result = self.assertJSONResponse(reset_response, status.HTTP_200_OK)
        self.assertFalse(reset_result["has_custom_name"])

    def test_post_scenario_rule_with_has_custom_name(self):
        """
        has_custom_name can be set to True at creation time.
        """
        self.client.force_authenticate(user=self.user_with_full_perm)
        ...
        result = self.assertJSONResponse(response, status.HTTP_201_CREATED)
        self.assertTrue(result["has_custom_name"])
        self.assertTrue(ScenarioRule.objects.get(id=result["id"]).has_custom_name)
