...
class ScenarioRulePermission(permissions.BasePermission):
    def has_permission(self, request, view):
        user = request.user
        if not user or not user.is_authenticated:
            return False

        if request.method in permissions.SAFE_METHODS:  # = GET for list, retrieve
            return True
    ...
    def has_object_permission(self, request, view, obj: ScenarioRule):
