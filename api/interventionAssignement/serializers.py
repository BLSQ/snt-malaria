...
class InterventionAssignmentSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if missing_org_units:
            raise serializers.ValidationError({"orgunit_ids": f"Invalid org_unit IDs: {missing_org_units}"})
        ...
        if missing_interventions:
            raise serializers.ValidationError({"intervention_ids": f"Invalid intervention IDs: {missing_interventions}"})
